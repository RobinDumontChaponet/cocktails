...
<div id="content">
	<form action="login" method="post" name="connection">
		<?php if($data['bot']===true) echo'<p class="mapsitna">Accès interdit !</p>';
			...
			<input title="Identifiant" id="user" name="user" type="text" value="<?php echo isset($_POST['user'])?$_POST['user']:'' ?>" required autofocus />
			...
			<br /><br /><?php /* f****/ ?>
			<input class="<?php echo ($data['badInput']!==true)?'ok':'warning' ?>" name="submit" type="submit" value="connexion" />
